...
int main() {
    ...
    for (int i = 0; i < Count; ++i) {
        for (int j = 0; j < Count; ++j) {
            ...
            thread th2(UpdateCurrentMax,std::ref(ans_sum),std::ref(ans_i),std::ref(ans_j),saved_max,saved_i,saved_j);

            th2.join();
            thread th3(Console,saved_i,saved_j,saved_max);
            th3.join();
        }
        cout<<endl;
    ...
    cout<<"\n\n\n"<<"Current max: "<<ans_sum<<endl<<"Best i: "<<ans_i<<endl<<"Best j: "<<ans_j<<endl;
